...
const EditAlarm = () => {
  const { alarmArr, setAlarmArr, setShowSettings, setSettingsType, toEdit, setToEdit } = useContext(contextData);

  const [repeat, setRepeat] = useState(toEdit.repeat);  
  const [del, setDel] = useState(false);

  const [snoozeTxt, setSnoozeTxt] = useState("");
  const [toneTxt, setToneTxt] = useState("");

  const [daysArr, setDaysArr] = useState(toEdit.days)
  const [txt, seTxt] = useState(`${toEdit.title}`)

  const [snoozeMenu, setSnoozeMenu] = useState(false);
  const [toneMenu, setToneMenu] = useState(false);
  const [snoozeVal, setSnoozeVal] = useState(toEdit.snooze);
  const [toneVal, setToneVal] = useState(toEdit.tone);

  const [num1, setNum1] = useState(toEdit.h%12)
  const [num2, setNum2] = useState(toEdit.m)
  const [num3, setNum3] = useState(toEdit?.h > 11 ? 1 : 0)
  ...
  const saveHandle = () => {
    ...
    setAlarmArr(
      alarmArr.map(element=>{
        if (element.id === toEdit.id) {
          return{
            ...element,  title: txt, time: { h: hr, m: num2 }, active: true, days: daysArr, tone: toneVal, repeat: repeat, snooze: snoozeVal 
          }
        }
        return element
      })
  ...
  const handleDelete = () =>{
    if (alarmArr.length > 1) {
      if (window.confirm("Do you want to delete this Timer Element?")) {
        setAlarmArr(alarmArr.filter(item => item.id !== toEdit.id));
        cancelHandler()
      }
    }
  }
